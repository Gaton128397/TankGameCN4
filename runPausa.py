...
def run_pausa():
    ...
    pausa_background = pygame.image.load('pausa.png')  
    pausa_background = pygame.transform.scale(pausa_background, (width, height))
    screen.blit(pausa_background, (0, 0))

    clock = pygame.time.Clock()

    home_button = button.Button((width // 2 - (x*4 + 10) // 2, height // 2 - x*0.45 - 100, x*4 + 10, x*0.75), (255, 0, 0), 'Home', False)
    settings_button = button.Button((width // 2 - (x*4 + 10) // 2, height // 2 - x*0.45 , x*4 + 10, x*0.75), (255, 0, 0), 'Settings', False)
    controls_button = button.Button((width // 2 - (x*4 + 10) // 2, height // 2 + 100, x*4 + 10, x*0.75), (0, 0, 255), 'Controls', False)
