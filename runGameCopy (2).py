...
import pygame,nTank,nTerrain,sys, params, drawFunctions, player, random, playerPhysics, ninfoBlock, functions, nProyectil, time
...
class gameLogic:
    ...
    def __init__(self, windowGame,playerList):
        ...
        drawFunctions.backgroundDraw(self.backGround,params.bgSelva)
    ...
    def run(self,clock):
        ...
        playerPhysics.playerSpawn(self.screen,self.listaJugadores,self.terrain,drawFunctions.returnSurface([self.backGround,self.terrain.surfTerrain]),9.8)
                            ...
                            if event.key == pygame.K_RETURN: #recien aqui recibe la potencia para disparar
                                ...
                                if potencia >0:
                                    if self.listaPlayers[jugador.playerID].inventory[balaID] > 0:
                                        ...
                                        bullet = nProyectil.Projectile((int(jugador.getPos()[0]+jugador.xCanon2-(params.WIDTH*0.025)),int(jugador.getPos()[1]+jugador.yCanon2-(params.HEIGHT*0.02))),balaID,100,jugador.angulo,self.screen,self.listaJugadores,9.8,1)
                                        ...
                                        playerPhysics.fallTanks(self.screen,self.listaJugadores,self.terrain.getDiccionary(),drawFunctions.returnSurface([self.backGround,self.terrain.surfTerrain]),9.8)
                                        turnos[0] = -1
                                    else:
                                        print('no quedan')
